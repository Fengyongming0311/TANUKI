...
from Case28_ShoppingCart_DeleteGoods import DeleteGoods


class ShoppingCart_Delete(unittest.TestCase):
    @classmethod
    ...
    def test_ShoppingCart_002ChooseGoods(self):
        ...
        driver = self.driver
        TestResult = ChooseGoods.ChooseGoods(driver)
        # 实地打造师小程序→购物车→勾选商品
        self.assertTrue(TestResult)
        # print ("选择智能场景测试用例执行完毕")

    # @unittest.skip("调试，不执行这条用例")
    def test_ShoppingCart_003DeleteGoods(self):
        """用例名称:实地打造师小程序→购物车→删除商品"""
        driver = self.driver
        TestResult = DeleteGoods.DeleteGoods(driver)
        # 实地打造师小程序→购物车→删除商品
        self.assertTrue(TestResult)
