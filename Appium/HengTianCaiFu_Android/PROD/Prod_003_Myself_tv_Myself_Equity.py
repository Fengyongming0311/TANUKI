...
class Test_HengTian_Myself_Equity:
    ...
    #@pytest.mark.skip(reason="不执行这条用例")
    @allure.feature('用例名称:会员权益→MGM有礼')
    def test_Myself_Equity_008VIPRoom(self):
        ...
        """
        driver = Driver.driver
        AllEquitys.EveryEquitys(driver, "MGM有礼")
        TestResult = AllEquitys.EveryEquitys_CheckPoint(driver, "MGM有礼")
        assert TestResult == True
...
if __name__ == "__main__":
    ...
    if int(ENV) == 1:
        ...
        pytest.main(['Prod_003_Myself_tv_Myself_Equity.py', "-s", "-x", "-v"])
