...
class Test_Bottom_tv:
    ...
    #@pytest.mark.skip(reason="不执行这条用例")
    @allure.feature('用例名称:APP首页→了解恒天→风控体系')
    def test_Bottom_tv_004RiskControlSystem(self):
        """
        """
        driver = Driver.driver
        TestResult = bottom_tv.grid_item(driver, "风控体系")
        assert TestResult == True


    #@pytest.mark.skip(reason="不执行这条用例")
    @allure.feature('用例名称:APP首页→了解恒天→许可证')
    def test_Bottom_tv_005Licence(self):
        """
        """
        driver = Driver.driver
        TestResult = bottom_tv.grid_item(driver, "许可证")
        assert TestResult == True
...
if __name__ == "__main__":
    ...
    if int(ENV) == 1:
        ...
        pytest.main(['start_022_HP_Bottom_tv.py', "-s", "-v"])
