...
class IntoSiMuProduct:
    ...
    def IntoSiMuProduct_CheckPoint(driver):
        try:
            with allure.step('截取私募产品页面截图'):
                time.sleep(2)
    ...
    def TiaozhuanMarket(driver):
        try:
            with allure.step('点击去市场看看'):
                time.sleep(3)
                Page.wait_elem(driver,"com.chtwm.mall:id/bt_suction_bottom",21,3).click()
                time.sleep(3)
                allure.attach(driver.get_screenshot_as_png(),'%s' %PrtScPath,attachment_type=allure.attachment_type.PNG)
                #添加检查点截图，PrtScPath为图片位置
