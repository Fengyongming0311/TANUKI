...
def GetWinningInfo(webaddress):
    ...
    # 包括每一期的期号；开奖日期等信息
    resultinfo_haomalist = str(resultinfo_haoma).replace('<em class="rr">', '').replace('</em>', '').replace('<em>',
                                                                                                             '').replace(
        '[', '').replace(']', '').split(',')
...
for e in range(1, numberOfPages + 1):
    ...
    for everyIssuerData in resultall:
        ...
        # 小细节把抓取的数据写入前转换成数字格式
        cellValue = [everyIssuerData[0], int(everyIssuerData[1]), int(everyIssuerData[2][0]),
                     int(everyIssuerData[2][1]), int(everyIssuerData[2][2]), int(everyIssuerData[2][3]),
                     int(everyIssuerData[2][4]), int(everyIssuerData[2][5]), int(everyIssuerData[2][6])]
        if (i % 2) == 0:
