...
def main():
	...
	data = xlrd.open_workbook(filename, encoding_override = 'GBK')  #打开文件

	#data = xlrd.open_workbook(filename, encoding_override = 'ANSI')  #打开文件

	#listname = config.get("list", "list")

	biao1 = data.sheet_by_index(0)  #取第一个表
	...
	#在这转一下码
	allstaff = name_zhuanma(allstaff)
	print (allstaff)
